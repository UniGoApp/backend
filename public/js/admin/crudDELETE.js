...
const deleteData = (e) => {
    ...
    const modal_title = document.querySelector('#modalDelete h4').innerText.toLowerCase().split(': ');
    let targetAPI = modal_title[0];
    let targetCompleteID = modal_title[0]+'-'+modal_title[1];
    ...
    .then(result => {
        ...
        if(result.error){
            ...
            // Delete row
            document.querySelector(`#bbdd-${targetAPI} tbody > tr#${targetCompleteID}`) .style.display="none";
            // Show notification 
