...
const putUsuarios = async (req, res) => {
    if(req.auth._rol === "SUPER_ADMIN" || req.auth._rol === "ADMIN"){
        con.execute(
            'UPDATE usuarios SET email=?, username=?, rol=?, phone=?, picture=?, reset_code=?, creation_time=?, rrss=? WHERE id=?;', [req.body.email, req.body.username, req.body.rol, req.body.phone, req.body.picture, req.body.reset_code, req.body.creation_time, req.body.rrss, req.params.id], (err, result) => {
                if (err) console.log('err :>>', err);
                con.execute(
